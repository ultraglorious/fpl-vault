...
uv run python ingest.py     # Ingest bootstrap-static into fpl_dev.duckdb
uv run python ingest.py --live  # Ingest into fpl.duckdb (with backup)
```
...
- **`ingest.py`** — Orchestrates fetching, schema inference, table creation, and data insertion. Targets `fpl_dev.duckdb` by default; `--live` flag targets `fpl.duckdb` with automatic backup.
