...
Table schemas are auto-generated by `--init` and written to `schema/fpl_api/` (configurable via `SCHEMA_DIR`). This directory is gitignored — each environment generates its own from the live API.

Example schema file (`schema/fpl_api/teams.yml`):
...
Only primary key columns get `tests: [not_null]`. Everything else defaults to nullable to avoid constraint failures on API drift.
